...
def int_cmp(x, y):
    ...
    #print int(1000.*(x.time - y.time))
    x_time = round(x.time, 5)
    y_time = round(y.time, 5)
    if x_time > y_time:
        return 1
    if x_time < y_time:
        return -1
    ...
    if x.ord > y.ord:
        return 1
    return 0
    #return int( 1000.0 * (x.time - y.time))/1000
class MIDIEvent:
    '''
    ...
    def __str__(self):
        ...
        return "Type: {}, time: {}, ord: {}".format(self.type, self.time, self.ord)
    ...
    def __eq__(self, other):
        ...
        if self.type == 'tempo':
            if self.tempo != other.tempo:
                return False
        if self.type == 'timesignature':
            if self.num != other.num or self.denom != other.denom or self.ticks != other.ticks or self.n32rd != other.n32rd:
                return False
        if self.type == 'keysignature':
            if self.accidentals != other.accidentals or self.maj_marker != other.maj_marker:
                return False
...
class MIDITrack:
    ...
    class timeSignature(GenericEvent):
        '''A class that encapsulates a time signature meta-event
        '''
        def __init__(self, time, num, denom, ticks=24, n32rd = 8):
    ...
    class keySignature(GenericEvent):
        '''A class that encapsulates a key signature meta-event
        '''
        def __init__(self, time, accidentals=0, maj_marker=0):

            GenericEvent.__init__(self, time)
            self.type = 'keySignature'
            self.accidentals= accidentals
            self.maj_marker = maj_marker 
    ...
    def addControllerEvent(self, channel, time, eventType, parameter1):
        '''
    ...
    def addTimeSignature(self,time,num, denom, ticks=24, n32rd =8):
        '''
        Add a time signature change (or set) event.
        '''
        self.eventList.append(MIDITrack.timeSignature(time, num, denom, ticks, n32rd))

    def addKeySignature(self, time, accidentals, maj_marker):
        '''
        Add a key signature change (or set) event.
        '''
        self.eventList.append(MIDITrack.keySignature(time, accidentals, maj_marker))
    ...
    def processEventList(self):
        ...
        for thing in self.eventList:
            if thing.type == 'note':
                ...
                event.ord = 2
                self.MIDIEventList.append(event)
                #print "NoteOn: {} NoteOff: {}, pitch: {}".format(thing.time * self.ticksPerBeat, event.time, thing.pitch)
                ...
                event.time = thing.time * self.ticksPerBeat
                event.num = thing.num
                event.denom = thing.denom
                event.ticks = thing.ticks
                event.n32rd = thing.n32rd
                event.ord = 3
                self.MIDIEventList.append(event)
                ...
                event.maj_marker = thing.maj_marker
                event.ord = 3
                self.MIDIEventList.append(event)
        ...
        self.MIDIEventList.sort(key = functools.cmp_to_key(int_cmp))
        
        if self.deinterleave:    
            self.deInterleaveNotes()
        #last_event= None
        #print "Process called"
        #for i, ev in enumerate(self.MIDIEventList):
            #if last_event is not None:
            #    print "{}: {} -> {}".format(i, last_event.type, ev.type)
            #if last_event is not None and ev.type == last_event.type:
            #    print "interleave found! IDs {} {}".format(i-1, i)
            #    print ev.__dict__
    ...
    def removeDuplicates(self):
        ...
        self.eventList.sort(key = functools.cmp_to_key(lambda x, y: cmp(x.type,  y.type)))
        self.eventList.sort(key = functools.cmp_to_key(lambda x, y: int( 1000 * (x.time - y.time)))) #A bit of a hack.
    ...
    def writeEventsToStream(self):
        ...
        '''
        #times = [ev.time for ev in self.MIDIEventList]
        #print len(times), len(set(times))
        #tmp = {"TrackName": "", "KeySignature": "", "Tempo": "", "TimeSignature":"", "NoteOn": "1", "NoteOff":"0"}
        #types = [tmp[ev.type] for ev in self.MIDIEventList if ev.type in tmp.keys()]
        #print "".join(types)
        ...
        for event in self.MIDIEventList:
            if event.type == "NoteOn":
                ...
                self.MIDIdata = self.MIDIdata + struct.pack('>B', int(event.pitch))
                self.MIDIdata = self.MIDIdata + struct.pack('>B', int(event.volume))
                ...
                self.MIDIdata = self.MIDIdata + struct.pack('>B', code)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', subcode)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', 0x04) # Data length: 4
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.num) 
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.denom) 
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.ticks) 
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.n32rd) 
                ...
                self.MIDIdata = self.MIDIdata + struct.pack('>B', code)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', subcode)
                self.MIDIdata = self.MIDIdata + struct.pack('>B', 0x02) # Data length: 2
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.accidentals) 
                self.MIDIdata = self.MIDIdata + struct.pack('>B', event.maj_marker) 
                ...
                self.MIDIdata = self.MIDIdata + event.trackName.encode("ISO-8859-1")
    ...
    def deInterleaveNotes(self):
        ...
        self.MIDIEventList.sort(key = functools.cmp_to_key(lambda x, y: cmp(x.type,  y.type)))
        self.MIDIEventList.sort(key = functools.cmp_to_key(int_cmp))
...
class MIDIHeader:
    ...
    def __init__(self, numTracks, ticksPerBeat):
        ...
        if ticksPerBeat > 65535:
            raise ValueError("Ticks per beat to Large: {} (0 <= ticks < 65536) ".format(ticksPerBeat))
...
class MIDIFile:
    ...
    Calling:
        ...
        MyMIDI = MidiFile(tracks, ticksPerBeat=TICKSPERBEAT, removeDuplicates=True,  deinterleave=True)
    ...
    def __init__(self, numTracks, ticksPerBeat=TICKSPERBEAT, removeDuplicates=True,  deinterleave=True):
        '''
    ...
    def addTimeSignature(self, track, time, num, denom, ticks=24, n32rd = 8):
        """
        Add a time signature event.
        
        Use:
            MyMIDI.addTimeSignature(track, time, num, denom, ticks, n32rd = 8)
            
        Arguments:
            track: The track to which the event is added. [Integer, 0-127].
            time: The time at which the event is added, in beats. [Float].
            num: The numerator of the time signature. [Integer]
            denom: The denominator of the time signature. [Integer]
            ticks: Number of ticks per quarter note (PPQ). [Integer]
            n32rd: Number 32rd-note per quarter note. [Integer, Default: 8]
        """
        self.tracks[track].addTimeSignature(time, num, denom, ticks, n32rd)

    def addKeySignature(self, track, time, accidentals=0, maj_marker=0):
        """
        Add a key signature event.
        
        Use:
            MyMIDI.addKeySignature(track, time, accidentals = 0, maj_marker = 0)
            
        Arguments:
            track: The track to which the event is added. [Integer, 0-127].
            time: The time at which the event is added, in beats. [Float].
            accidentals: Number of flats (-7 to -1) or sharps (1 to 7) or 0 (C-Major). [-7, +1]
            maj_marker: 0=major, 1=minor. [0, 1,]
        """
        self.tracks[track].addKeySignature(time, accidentals, maj_marker)
    ...
    def close(self):
        ...
        for i in range(0, self.numTracks):
            ...
            self.tracks[i].MIDIEventList.sort(key= functools.cmp_to_key(int_cmp))
